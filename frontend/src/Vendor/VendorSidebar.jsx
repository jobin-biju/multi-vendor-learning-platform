...
import { Menu, ChevronLeft, ChevronRight, Book, PlusCircle, ShoppingCart, User, LogOut, BarChart3, Bell, MessageCircleCode,MessageSquare  } from 'lucide-react';
...
import ViewReviewIns from './ViewReviewIns';
...
const VendorSidebar = () => {
  ...
  const menuItems = [
    ...
    { id: 'mycourse', icon: <Book className="w-5 h-5" />, label: 'My Courses' },
    { id: 'add-course', icon: <PlusCircle className="w-5 h-5" />, label: 'Add Course' },
    { id: 'add-instructor', icon: <PlusCircle className="w-5 h-5" />, label: 'Add Instructor' },
    { id: 'view-instructor', icon: <PlusCircle className="w-5 h-5" />, label: 'View Instructor' },
    { id: 'orders', icon: <ShoppingCart className="w-5 h-5" />, label: 'Orders' },
    { id: 'Review', icon: <MessageSquare  className="w-5 h-5" />, label: 'Review' },
    // { id: 'Chat', icon: <MessageCircleCode className="w-5 h-5" />, label: 'Chat' }
  ...
  const renderContent = () => {
    ...
    if (activeMenu === 'dashboard') {
      return <VendorDash/>;
    }else if (activeMenu  === 'add-course'){
      return <AddCourse/>;
    }else if (activeMenu === 'add-instructor'){
      return <AddInstructor/>
    }else if (activeMenu === 'view-instructor'){
      return <ViewInstructor/>
    }else if (activeMenu === 'mycourse'){
      return <Mycourse/>
    }else if (activeMenu === 'orders'){
      return <Orders/>
    }else if (activeMenu === 'Review'){
      return <ViewReviewIns/>
    }
          ...
          {!collapsed && (
            <div className="flex items-center group">
              ...
              <h5 className="font-bold text-lg ml-3 text-blue-50">
                Vendor<span className="text-blue-300 animate-pulse">Hub</span>
              </h5>
            </div>
          )}
          <button 
